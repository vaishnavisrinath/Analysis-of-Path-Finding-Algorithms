...
for size in input_sizes:
    random_graph = generate_random_graph(size, size * 2)

    start_time = time.time()
    dijkstra(random_graph, 0)
    dijkstra_time = time.time() - start_time
    dijkstra_times.append(dijkstra_time)
    print(f"Dijkstra's Algorithm for {size} nodes took: {dijkstra_time:.4f} seconds")

    start_time = time.time()
    astar(random_graph, 0, size - 1, int(size ** 0.5))
    astar_time = time.time() - start_time
    astar_times.append(astar_time)
    print(f"A* Search for {size} nodes took: {astar_time:.4f} seconds")

    start_time = time.time()
    bfs(random_graph, 0, size - 1)
    bfs_time = time.time() - start_time
    bfs_times.append(bfs_time)
    print(f"BFS for {size} nodes took: {bfs_time:.4f} seconds")

    start_time = time.time()
    dfs(random_graph, 0, size - 1)
    dfs_time = time.time() - start_time
    dfs_times.append(dfs_time)
    print(f"DFS for {size} nodes took: {dfs_time:.4f} seconds")
...
plt.title('Performance Comparison of Graph Search Algorithms (Random Graph)')
plt.legend()
plt.grid(True)
plt.show()
